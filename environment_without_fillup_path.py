...
# 这里的5个py文件思路如下：JPS生成控制点，不进行点间填充，同时删除path的中间点，最后直接用B-spline
# 大多数时候，是撞障碍物的的。（这主要是因为jps控制点生成的不好的原因）
...
def pop_middle(gridmap, path):
    ...
    while True:
        ...
        index =0
        while True:
            if len(path) - 1 <= index + 1:
                break
            dx = np.linspace(path[index][0], path[index+2][0], 20)
            dy = np.linspace(path[index][1], path[index+2][1], 20)
            for i in range(len(dx)):
                if gridmap[round(dx[i])][round(dy[i])] == 1:
                    index = index + 1
                    break
            else:
                path.pop(index + 1)
...
if __name__ =='__main__':
    ...
    path = game.path
    new_path = pop_middle(game.map_class.get_map(),path)
    x=[]
    y=[]
    for pos in new_path:
        x.append(pos[0])
